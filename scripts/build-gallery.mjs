...
for (const dirent of archiveDirs) {
  ...
  const imageFile =
    configuredImage && files.includes(configuredImage)
      ? configuredImage
      : files.find((file) => imageExtensions.has(path.extname(file).toLowerCase()));
  ...
  entries.push({
    ...
    imageSrc: imageFile ? `../data/archive/${id}/${imageFile}` : "",
    imageFile: imageFile || "",
