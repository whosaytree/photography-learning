...
3. Before judging any rotation/orientation problem, actively check the archive metadata and the local image file for available orientation information. If EXIF orientation or trusted gallery/user display information is available, judge the photograph after that orientation is applied; never use the raw pixel matrix alone to mark a rotation error. If no orientation information is available, use the correctly oriented user/gallery display when present. Treat this as an internal check: do not state a specific EXIF orientation value in the critique unless it was actually read from the file or metadata.
